...
| `apis.coap.ownershipTransfer.method` | string | `Selected ownership transfer method. The supported values are: "justWorks", "manufacturer".` | `"justWorks"` |
| `apis.coap.ownershipTransfer.manufacturer.tls.caPool` | string | `File path to the root manufacturer certificate in PEM format which might contain multiple certificates in a single file.` |  `""` |
| `apis.coap.ownershipTransfer.manufacturer.tls.keyFile` | string | `File path to manufacturer client application private key in PEM format.` | `""` |
| `apis.coap.ownershipTransfer.manufacturer.tls.certFile` | string | `File path to manufacturer client application certificate in PEM format.` | `""` |
